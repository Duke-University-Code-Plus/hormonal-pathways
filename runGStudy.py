...
def runGStudy():
    ...
    for j in range(20):
        ...
        while alive and i < N - 1:
            ...
            X_t1, S_t1, C_t1, W_t1 = forwardModel(Xhist[0, i, j], beta_t, z, Shist[:, i, j], Chist[0, i, j], K, E_t, gamma, delCmax, delSmax, Xmin, G[j])
            Xhist[0, i + 1, j] = X_t1
            Shist[:, i + 1, j] = S_t1
            Chist[0, i + 1, j] = C_t1
            Whist[0, i + 1, j] = W_t1
...
def forwardModel(X_t, beta_t, z_t, S_t, C_t, K, E_t1, gamma_t, delCmax, delSmax, Xmin, G):
    ...
    results = minimize(fitness_wrapper, np.zeros(4), bounds=bounds)
    delMaxCS = results.x
