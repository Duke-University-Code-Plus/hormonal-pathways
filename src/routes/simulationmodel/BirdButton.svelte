...
const createSketch = () => {
        new p5((p) => {
            ...
            if (Birdiff == "2") {
                p.preload = () => {p.loadJSON('Bird/malebird2_fly.json', function(malebird_fly_frames) {
                        malebird_fly_spritesheet = p.loadSpriteSheet('Bird/malebird2_fly_spritesheet.png', malebird_fly_frames);
                    });
                }
            }

            if (Birdiff == "3") {
                p.preload = () => {p.loadJSON('Bird/malebird1_fly.json', function(malebird_fly_frames) {
                        malebird_fly_spritesheet = p.loadSpriteSheet('Bird/malebird1_fly_spritesheet.png', malebird_fly_frames);
                    });
                }
            }
            ...
            p.setup = () => {
                ...
                if(Birdiff == "2") {
                malebird_fly = p.loadAnimation(malebird_fly_spritesheet);
                malebird.addAnimation('stand', 'Bird/malebird2_stand.png');
                }
                if(Birdiff == "3") {
                malebird_fly = p.loadAnimation(malebird_fly_spritesheet);
                malebird.addAnimation('stand', 'Bird/malebird1_stand.png');
                }
