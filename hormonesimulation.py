...
def hormoneSimulation(gammaIn : np.array = np.array([.1, 2]), 
                 ...
                 zIn : np.array = np.array([0.2, 0.3]), NIn : int = 100,
    ...
    Xhist[0] = 1
    Shist[:, 0] = np.array([1, 1])
    Chist[0] = 1
    Whist[0] = 1
    Vhist[:, 0] = np.array([1, 1])
...
def forwardModel(X_t, beta_t, z_t, S_t, C_t, K, E_t1, gamma_t, delCmax, delSmax, Xmin, G):
    ...
    W_t1 = beta_t * (V_t[0]**z_t[0]) * (V_t[1]**z_t[1]) 
                                                        #* (V_t[2]**z_t[2])
