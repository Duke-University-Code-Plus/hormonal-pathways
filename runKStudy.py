...
def runKStudy():
    ...
    for j in range(20):
        ...
        while alive and i < N - 1:
            ...
            X_t1, S_t1, C_t1, W_t1 = forwardModel(Xhist[0, i, j], beta_t, z, Shist[:, i, j], Chist[0, i, j], K[j], E_t, gamma, delCmax, delSmax, Xmin, G)
            Xhist[0, i + 1, j] = X_t1
            Shist[:, i + 1, j] = S_t1
            Chist[0, i + 1, j] = C_t1
            Whist[0, i + 1, j] = W_t1
...
def forwardModel(X_t, beta_t, z_t, S_t, C_t, K, E_t1, gamma_t, delCmax, delSmax, Xmin, G):
    def fitness_wrapper(delCS):
        return fitness(beta_t, z_t, S_t, C_t, K, X_t, E_t1, gamma_t, delCS, Xmin, G)

    delMaxCS = minimize(fitness_wrapper, np.zeros(4), bounds=[(-delSmax, delSmax), (-delSmax, delSmax), (-delSmax, delSmax), (-delCmax, delCmax)])

    for idx in range(3):
        if S_t[idx] + delMaxCS[idx] < 0:
            delMaxCS[idx] = -S_t[idx]

    V_t = np.zeros_like(S_t)
    for idx in range(3):
        V_t[idx] = (S_t[idx] + delMaxCS[idx]) * (C_t + delMaxCS[3]) / (K + (C_t + delMaxCS[3]))

    X_t1 = X_t + E_t1 - np.dot(gamma_t, V_t)
    W_t1 = beta_t * np.prod([V_t[idx]**z_t[idx] for idx in range(3)])
...
def fitness(beta, z, S, C, K, X_t, E_t1, gamma, delCS, Xmin, G):
    V = np.zeros_like(S)
    for idx in range(3):
        V[idx] = (S[idx] + delCS[idx]) * (C + delCS[3]) / (K + (C + delCS[3]))
        if S[idx] + delCS[idx] < 0:
            V[idx] = 0

    W_t1 = beta * np.prod([abs(V[idx])**z[idx] for idx in range(3)])
    X_t1 = X_t + E_t1 - np.dot(gamma, V)

    return -W_t1  # Assuming we want to minimize the negative fitness
